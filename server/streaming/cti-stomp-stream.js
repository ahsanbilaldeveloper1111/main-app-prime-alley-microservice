...
export default function handler(req, res) {
  ...
  // Check if CTI server URL is configured. Accept VITE_* (current SPA env
  // schema) with NEXT_PUBLIC_* fallback for backward compatibility.
  const ctiHostEnv =
    process.env.VITE_PRIVATE_CTI_SOCKET_URL ||
    process.env.NEXT_PUBLIC_PRIVATE_CTI_SOCKET_URL;
  if (!ctiHostEnv) {
    res.write(`data: ${JSON.stringify({ type: 'error', status: 'error', message: 'CTI server URL not configured. Set VITE_PRIVATE_CTI_SOCKET_URL (or NEXT_PUBLIC_PRIVATE_CTI_SOCKET_URL) in the streaming sidecar env.' })}\n\n`);
    res.end();
    return;
  }
  ...
  const ctiServerHost = ctiHostEnv;
