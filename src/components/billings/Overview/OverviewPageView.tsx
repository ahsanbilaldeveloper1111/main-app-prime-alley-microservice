...
import { billingHelpLinks, starterIncludes } from "./overviewConstants";
import { overviewStyles as styles } from "./overviewStyles";
import type { BillingOverviewPageContext } from "./useBillingOverviewPage";

export type OverviewPageViewProps = Readonly<{
  ctx: BillingOverviewPageContext;
}>;

export function OverviewPageView({ ctx }: OverviewPageViewProps) {
  const {
    session,
    companyDetails,
    ...
    <>
      <div style={styles.card}>
        <div style={{ ...styles.cardPadding, display: "flex", alignItems: "flex-start", gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <h2 style={styles.companyHeading}>{session?.user?.company_name}</h2>
            <TopSection
        ...
        <div style={{ ...styles.card, marginBlockEnd: 0 }}>
          <div style={styles.cardPadding}>
            ...
            <p style={{ margin: "0 0 16px 0", fontSize: 18, color: "#141414" }}>
              ...
              <strong>
                AED {companyDetails?.latest_invoice?.total_amount ?? "0"}
              </strong>{" "}
              will be charged on the 1st of every month.
            </p>
            <p style={{ margin: "0 0 16px 0", fontSize: 12, color: "#666", lineHeight: "18px" }}>
              *Includes estimated sales tax or VAT, based on your main company address. Excludes any recent
              credits to your account. Your recurring fees may increase based on your usage.
            </p>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ ...styles.label, marginBottom: 4 }}>
                <strong style={{ color: "#141414", fontSize: 14 }}>Products &amp; Add-ons</strong>
              </div>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 14 }}>
                Visit the <Link href="/settings" style={styles.link}>Knowledge Base</Link> for answers to FAQs
                or <Link href="/settings" style={styles.link}>contact us</Link> for more support.
              </p>
            </div>
          </div>
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column" as const, gap: 14 }}>
              {visibleCommonActions.map((action) => (
                <div key={action.label} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <Link href={action.url} style={styles.link}>
                    {action.label}
                  </Link>
                </div>
              ))}
          ...
          <div style={styles.cardPadding}>
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button type="button" style={styles.btnLight}>
                  Manage usage
                </button>
              </div>
            </div>
            ...
            <div style={{ height: 8, backgroundColor: "#e5e5e5", borderRadius: 4, marginBottom: 8 }}>
              <div
                style={{ width: "0%", height: "100%", backgroundColor: "#00a47c", borderRadius: 4 }}
              />
            </div>
            <p style={{ fontSize: 12, color: "#666", margin: "0 0 12px 0" }}>Resets on 10 Mar 2026.</p>
            <div
              style={{
                border: "1px solid rgb(0, 97, 98)",
                borderRadius: 4,
                padding: "8px 12px",
                fontSize: 12,
                color: "rgb(0, 97, 98)",
        ...
        <div style={{ ...styles.card, marginBlockEnd: 0 }}>
          <div style={styles.cardPadding}>
            ...
            <p style={{ fontSize: 13, color: "#444", lineHeight: "20px", margin: 0 }}>
              Billed data for marketing contacts is updated once a day. Marketing contacts are updated to
              non-marketing once a month on the next update date. Go to{" "}
              <Link href="/billing/account-billing" style={styles.link}>
                Usage &amp; Limits
              </Link>{" "}
        ...
        <div style={styles.cardPadding}>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button type="button" style={{ ...styles.btnLight, gap: 6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px 32px" }}>
            {billingHelpLinks.map((row) => (
              <div key={`${row[0]}|${row[1]}`} style={{ display: "contents" }}>
                <Link
                  href="/settings"
                  style={{ ...styles.link, fontSize: 13, display: "inline-flex", alignItems: "center", gap: 4 }}
                >
                  <span>{row[0]}</span>
