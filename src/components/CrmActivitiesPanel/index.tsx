...
function resolveAssignedToLabel(
  ...
  if (ext)
    return (
      (ext.display_name ?? ext.name ?? fallback(val)).trim() || fallback(val)
    );
  return fallback(val);
}
...
const AUDIT_FIELDS_EXTENSION = new Set<string>([
  "assigned_to",
  "contact_owner",
]);
...
export type AuditTrailExtension = {
  id?: number;
  extension?: string;
  display_name?: string;
  name?: string;
...
const CrmActivitiesPanelInnerRender: React.ForwardRefRenderFunction<
  CrmActivitiesPanelRef,
...
> = (
  ...
  const [selectedWhatsAppChatId, setSelectedWhatsAppChatId] = useState<
    number | null
  >(null);
  const [selectedWhatsAppChat, setSelectedWhatsAppChat] =
    useState<WhatsAppChatItem | null>(null);
  ...
  const [recordingModalError, setRecordingModalError] = useState<string | null>(
    null,
  );
  const [selectedRecordingForPlay, setSelectedRecordingForPlay] =
    useState<any>(null);
  ...
  const extensionNameMap = useMemo(() => {
    const list =
      (hierarchyDataExtensions as { id?: string; name?: string }[]) ?? [];
  ...
  const loadAuthenticatedRecordingAudio = useCallback(
    async (audioTrackId: string, agentExtension: string, node?: string) => {
  ...
  const handleWhatsAppReplySend = useCallback(async () => {
    ...
    try {
      ...
      const res = await getWhatsAppChatMessages({
        chat_id: String(selectedWhatsAppChatId),
      });
      const data = res as {
        messages?: WhatsAppMessage[];
        chat?: {
  ...
  const campaignsForAudit = useMemo(
    () =>
      campaigns ??
      (recordType === "prospect" || recordType === "lead"
        ? campaignsList
        : null),
  ...
  const activitiesData: ActivityItem[] = useMemo(() => {
    ...
    const resolveFieldVal = (field: string, raw: unknown): string => {
      if (AUDIT_FIELDS_EXTENSION.has(field))
        return resolveAssignedToLabel(raw, extensionsForAudit, fmt);
      if (field === "campaign_id")
        return resolveCampaignLabel(raw, campaignsForAudit, fmt);
      return fmt(raw);
    };
    return trail.map((entry) => {
      ...
      if (
        ...
        const changesObj = entry.changes as Record<
          string,
        ...
        Object.entries(changesObj).forEach(([field, val]) => {
          ...
          if (field === "data") {
            ...
            allKeys.forEach((key) => {
              ...
              if (o !== n)
                auditChanges.push({ field: key, oldVal: o, newVal: n });
            });
          } else {
            auditChanges.push({
              field,
              oldVal: resolveFieldVal(field, rawOld),
              newVal: resolveFieldVal(field, rawNew),
            });
          }
        });
      }
  ...
  const renderActivityItem = useCallback(
    (activity: ActivityItem) => {
      const isExpanded =
        expandedActivities.has(activity.id) || activity.expanded;
      return (
        <div
          key={activity.id}
          style={{
  ...
  if (minutesRemaining == null && whatsappChatWindowInfo?.window_started_at) {
    ...
    minutesRemaining = Math.max(
      0,
      Math.floor((start + 24 * 60 * 60 * 1000 - Date.now()) / 60000),
    );
  }
  const canSend =
    within24h && (minutesRemaining == null || minutesRemaining > 0);
  ...
  return (
    <div>
        ...
        <>
          {recordLoading ? (
            ...
            >
              ...
              >
                {recordName} history will appear here as the record is created
                and updated.
              </p>
            </div>
          ) : (
        ...
        <>
          {emailsLoading ? (
            ...
            >
              ...
              {emailsMeta &&
                (emailsMeta.total > 0 || emailsList.length > 0) && (
                  <div
                    style={{
                      display: "flex",
                      flexWrap: "wrap",
                    ...
                    {emailsMeta.last_page > 1 && (
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            setEmailsPage((p) => Math.max(1, p - 1))
                          }
                          style={{
                            padding: "6px 12px",
                            border: "1px solid #cbd5e0",
                            borderRadius: "4px",
                            fontSize: "13px",
                            cursor: emailsPage <= 1 ? "not-allowed" : "pointer",
                            backgroundColor: "#fff",
                            opacity: emailsPage <= 1 ? 0.6 : 1,
                          }}
                        >
                          Prev
                        </button>
                        {Array.from(
                          { length: Math.min(emailsMeta.last_page, 10) },
                          (_, i) => i + 1,
                        ).map((p) => (
                          <button
                            key={p}
                            type="button"
                            onClick={() => setEmailsPage(p)}
                            style={{
                              padding: "6px 12px",
                              border: "1px solid #cbd5e0",
                              borderRadius: "4px",
                              ...
                              cursor: "pointer",
                              backgroundColor:
                                p === emailsPage ? "#141414" : "#fff",
                              color: p === emailsPage ? "#fff" : "#141414",
                            }}
                          >
                            {p}
                          </button>
                        ...
                        <button
                          ...
                          onClick={() =>
                            setEmailsPage((p) =>
                              Math.min(emailsMeta.last_page, p + 1),
                            )
                          }
                          style={{
                            padding: "6px 12px",
                            ...
                            fontSize: "13px",
                            cursor:
                              emailsPage >= emailsMeta.last_page
                                ? "not-allowed"
                                : "pointer",
                            backgroundColor: "#fff",
                            opacity:
                              emailsPage >= emailsMeta.last_page ? 0.6 : 1,
                          }}
                        >
                          Next
                        </button>
        ...
        <>
          {notesLoading ? (
            ...
            >
              ...
              >
                Take notes about this record. You can even @mention a teammate
                if you need to.
              </p>
            </div>
          ) : (
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  if (
                                    ...
                                    return;
                                  try {
                                    await updateCrmNote(editingNoteId, {
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    await updateTask(editingTaskId, {
                                      ...
                                      due_date:
                                        editingTaskForm.due_date || undefined,
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          backgroundColor: "#fff",
                          opacity: smsPage <= 1 ? 0.6 : 1,
                        }}
                      >
                        Prev
                      </button>
                      {Array.from(
                        { length: smsMeta.last_page },
                        (_, i) => i + 1,
                      ).map((p) => (
                        <button
                          key={p}
                          type="button"
                          onClick={() => setSmsPage(p)}
                          style={{
                            padding: "6px 12px",
                            border: "1px solid #cbd5e0",
                            borderRadius: "4px",
                            fontSize: "13px",
                            cursor: "pointer",
                            backgroundColor: p === smsPage ? "#141414" : "#fff",
                            color: p === smsPage ? "#fff" : "#141414",
                          }}
                        >
                          {p}
                        </button>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setSmsPage((p) => Math.min(smsMeta.last_page, p + 1))
                        }
                        style={{
                          padding: "6px 12px",
                          border: "1px solid #cbd5e0",
                          ...
                          fontSize: "13px",
                          cursor:
                            smsPage >= smsMeta.last_page
                              ? "not-allowed"
                              : "pointer",
                          backgroundColor: "#fff",
                          opacity: smsPage >= smsMeta.last_page ? 0.6 : 1,
                        }}
                      >
                        Next
                      </button>
      ...
      {showRecordingModal && (
        ...
        >
          ...
          >
            ...
            {recordingModalLoading ? (
              <p style={{ margin: 0, color: "#64748b" }}>Loading audio…</p>
            ) : recordingModalError ? (
              <p style={{ margin: 0, color: "#b91c1c" }}>
                {recordingModalError}
              </p>
            ) : recordingModalAudioUrl ? (
              <AudioPlayer
                ref={recordingAudioPlayerRef}
                audioSrc={recordingModalAudioUrl}
                title={
                  selectedRecordingForPlay?.Id
                    ? `Recording ${selectedRecordingForPlay.Id}`
                    : "Recording"
                }
                showWaveform
                autoPlay
      ...
      {selectedEmailId != null &&
        (() => {
          const selectedEmail = emailsList.find(
            (e) => e.id === selectedEmailId,
          );
          if (!selectedEmail) return null;
          const html = getEmailPreviewHtml(selectedEmail.content);
          const toDisplay = Array.isArray(selectedEmail.to)
            ? selectedEmail.to.join(", ")
            : "—";
          const fromDisplay =
            extensionNameMap[selectedEmail.created_by ?? ""] ??
          ...
          >
            ...
            >
              <div>
                ...
                >
                  WhatsApp – {selectedWhatsAppChat.phone_number}
                </h3>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  {!canSend && whatsappChatWindowInfo != null && (
              ...
              {whatsappMessagesLoading ? (
                ...
                >
                  {whatsappMessages.map((msg) => {
                    ...
                    const displayText =
                      msg.message?.trim() ||
                      (msg.message_type === "template" ? "Template sent" : "—");
                    return (
                      <div
                        key={msg.id}
                        style={{
                      ...
                      >
                        <div
                          style={{
                            ...
                            padding: "10px 14px",
                            borderRadius: "12px",
                            backgroundColor: isOutbound ? "#141414" : "#f1f5f9",
                            color: isOutbound ? "#fff" : "#141414",
              ...
              >
                ...
                >
                  {whatsappReplyMessage.trim() && (
                    <button
                      type="button"
                      onClick={() => setWhatsappReplyMessage("")}
                      style={{
                        padding: "8px 14px",
                        backgroundColor: "#fff",
                  ...
                  <button
                    type="button"
                    onClick={() => void handleWhatsAppReplySend()}
                    disabled={
                      !canSend ||
                  ...
                  >
                    {whatsappReplySendLoading ? "Sending…" : "Send"}
                  </button>
                </div>
              </div>
            )}
      ...
      <DeleteConfirmationModal
        ...
        itemName={
          noteToDelete
            ? noteToDelete.text.length > 50
              ? `note "${noteToDelete.text.slice(0, 50)}…"`
              : `note "${noteToDelete.text}"`
            : undefined
        }
        itemType="note"
      ...
      <DeleteConfirmationModal
        ...
        itemName={
          meetingToDelete ? `meeting "${meetingToDelete.name}"` : undefined
        }
        itemType="meeting"
...
export const CrmActivitiesPanel = forwardRef<
  CrmActivitiesPanelRef,
  CrmActivitiesPanelProps
>(CrmActivitiesPanelInnerRender);
