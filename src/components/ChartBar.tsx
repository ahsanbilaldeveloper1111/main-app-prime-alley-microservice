...
const ChartBar: React.FC<ChartBarProps> = ({
  ...
  const shouldLimitItems =
    maxDisplayedItems && categories.length > maxDisplayedItems;
  ...
  const getTopIndices = (
    ...
    const indicesWithValues = categoryTotals.map((total, index) => ({
      index,
      total,
    }));
  ...
  const displayedCategories = shouldLimitItems
    ? getTopIndices(series, categories, maxDisplayedItems).map(
        (i) => categories[i],
      )
    : categories;
  ...
  const displayedSeries = shouldLimitItems
    ? series.map((s) => ({
        ...s,
        data: getTopIndices(series, categories, maxDisplayedItems).map(
          (i) => s.data[i],
        ),
      }))
    : series;
  ...
  const normalizeDataAcrossAllSeries = (
    series: ChartSeries[],
  ): ChartSeries[] => {
      ...
      y: {
        ...
        formatter: function (
          ...
          try {
            ...
            if (
              useLogScale &&
              displayedSeries[seriesIndex] &&
              dataPointIndex !== undefined
            ) {
              const originalValue =
                displayedSeries[seriesIndex].data[dataPointIndex] || 0;
        ...
        <Modal.Body>
          <div style={{ height: "400px" }}>
            <ReactApexChart
              ...
              series={
                useLogScale ? normalizeDataAcrossAllSeries(series) : series
              }
              type="bar"
              height={350}
            />
