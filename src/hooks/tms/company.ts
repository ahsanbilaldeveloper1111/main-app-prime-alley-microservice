...
import {
  ListCompanies,
  createUpdateCompanyCallingAccess as createUpdateCompanyCallingAccessAPI,
  createUpdateCompanyProfile,
  GetCompany,
  GetAvailableCompanyIccids,
  GetAvailableExtensions,
} from "@utils/tms/tmsProfiling";
import { toast } from "react-toastify";
import { generateFacCode } from "@utils/accounts";
...
export const useCompanyList = (params: IndexCompanyParams) => {
  ...
  useEffect(() => {
    ...
    if (
      memoizedParams.parent_id !== undefined ||
      memoizedParams.ids.length > 0
    ) {
      fetchData();
    }
...
export const useCreateUpdateCompanyCallingAccess = (onSuccess?: () => void) => {
  ...
  const updateCreateUpdateCompanyCallingAccess = async (
    data: CallingAccess,
  ) => {
    try {
      setIsLoading(true);
...
export const useGetAvailableCompanyIccids = (
  companyId: number,
  userId: number,
) => {
  const [data, setData] = useState<any>(null);
