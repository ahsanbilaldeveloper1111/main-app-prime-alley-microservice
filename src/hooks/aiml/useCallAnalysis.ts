...
type ChunksStateSetter = Dispatch<SetStateAction<any>>;
...
function buildDoneResultChunksPatch(result: any): Record<string, unknown> {
...
function upsertCompletedStepEntry(
  step: string,
...
export function useCallAnalysis() {
  ...
  const handleStepDataUpdate = (stepCode: string, result: any) => {
    ...
    if (run) {
      run(result, setChunksAnalysisData);
    } else {
