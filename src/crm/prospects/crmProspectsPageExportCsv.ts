...
const prospectsExportValueHandlers: Record<string, ExportValueHandler> = {
  campaign_id: (row) => {
...
export function formatProspectsExportRawValue(raw: unknown): string {
  if (raw == null) return "";
...
export function escapeProspectsCsvCell(val: string): string {
  const s = String(val);
  if (/[,"\n\r]/.test(s)) return `"${s.replace(/"/g, '""')}"`;
  return s;
}

export function getProspectsExportCellValue(
  row: any,
  header: string,
  nestedDataKeysSet: Set<string>,
): string | null {
  const specialHandler = prospectsExportValueHandlers[header];
  if (specialHandler) {
  ...
  return formatProspectsExportRawValue(raw);
}

const preferredProspectsExportTopLevelOrder = [
  "id",
  "name",
  "phone",
  "user_extension",
...
export function buildProspectsExportHeaders(allData: CrmDataItem[]): {
  headers: string[];
  nestedDataKeysSet: Set<string>;
} {
  ...
  const orderedTopLevel = [
    ...preferredProspectsExportTopLevelOrder.filter((k) => topLevelKeys.has(k)),
    ...Array.from(topLevelKeys)
      .filter((k) => !preferredProspectsExportTopLevelOrder.includes(k))
...
export function buildProspectsCsvContent(
  headers: string[],
  allData: CrmDataItem[],
  nestedDataKeysSet: Set<string>,
): string {
  return [
    headers.map((h) => escapeProspectsCsvCell(h)).join(","),
    ...allData.map((row) =>
      headers
        .map((h) =>
          escapeProspectsCsvCell(
            String(
              getProspectsExportCellValue(row, h, nestedDataKeysSet) ?? "",
            ),
          ),
        )
        .join(","),
...
export type ProspectSourceFileOption = { value: string; label: string };

/** Distinct non-empty `source_file` values from loaded rows for creatable select. */
export function buildProspectsSourceFileSelectOptions(
  dataList: readonly CrmDataItem[],
): ProspectSourceFileOption[] {
