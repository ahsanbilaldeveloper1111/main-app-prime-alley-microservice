...
export type ContactsListPaginationSlice = {
  currentPage: number;
  rowsPerPage: number;
  sortColumn: string;
...
/** Maps table filters + pagination to `getCrmData` params for the contacts list. */
export function buildContactsListCrmDataParams(
  memoizedFilters: Record<string, any>,
  pagination: ContactsListPaginationSlice,
...
/** Maps arbitrary filters to `getCrmData` params for contacts CSV export paging. */
export function buildContactsExportCrmDataParams(
...
export function computeContactsAdvancedFiltersApplied(
