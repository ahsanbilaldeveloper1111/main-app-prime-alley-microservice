...
const contactsExportValueHandlers: Record<string, ExportValueHandler> = {
  campaign_id: (row) => {
...
export function formatContactsExportRawValue(raw: unknown): string {
  if (raw == null) return "";
...
export function escapeContactsCsvCell(val: string): string {
  const s = String(val);
  if (/[,"\n\r]/.test(s)) return `"${s.replace(/"/g, '""')}"`;
  return s;
}

export function getContactsExportCellValue(
  row: any,
  header: string,
  nestedDataKeysSet: Set<string>,
): string | null {
  const specialHandler = contactsExportValueHandlers[header];
  if (specialHandler) {
  ...
  return formatContactsExportRawValue(raw);
}

const preferredContactsExportTopLevelOrder = [
  "id",
  "name",
  "phone",
  "user_extension",
...
export function buildContactsExportHeaders(allData: CrmDataItem[]): {
  headers: string[];
  nestedDataKeysSet: Set<string>;
} {
  ...
  const orderedTopLevel = [
    ...preferredContactsExportTopLevelOrder.filter((k) => topLevelKeys.has(k)),
    ...Array.from(topLevelKeys)
      .filter((k) => !preferredContactsExportTopLevelOrder.includes(k))
...
export function buildContactsCsvContent(
  headers: string[],
  allData: CrmDataItem[],
  nestedDataKeysSet: Set<string>,
): string {
  return [
    headers.map((h) => escapeContactsCsvCell(h)).join(","),
    ...allData.map((row) =>
      headers
        .map((h) =>
          escapeContactsCsvCell(
            String(
              getContactsExportCellValue(row, h, nestedDataKeysSet) ?? "",
            ),
          ),
        )
        .join(","),
...
export type ContactSourceFileOption = { value: string; label: string };

/** Distinct non-empty `source_file` values from loaded rows for creatable select. */
export function buildContactsSourceFileSelectOptions(
  dataList: readonly CrmDataItem[],
): ContactSourceFileOption[] {
