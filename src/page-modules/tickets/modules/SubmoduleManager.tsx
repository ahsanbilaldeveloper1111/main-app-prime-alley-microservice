...
const SubmoduleManager: React.FC<SubmoduleManagerProps> = ({
  show,
  onHide,
  module,
  extensions,
  ...
  const [newSubmoduleUserExtension, setNewSubmoduleUserExtension] =
    useState<any>(null);

  // Submodule child form states
  const [newSubmoduleChildName, setNewSubmoduleChildName] =
    useState<string>("");
  const [newSubmoduleChildDescription, setNewSubmoduleChildDescription] =
    useState<string>("");
  const [newSubmoduleChildColor, setNewSubmoduleChildColor] =
    useState<string>("#000000");
  const [newSubmoduleChildUserExtension, setNewSubmoduleChildUserExtension] =
    useState<any>(null);
  const [selectedSubmoduleForChildren, setSelectedSubmoduleForChildren] =
    useState<any>(null);
  ...
  const [editSubmoduleDescription, setEditSubmoduleDescription] =
    useState<string>("");
  const [editSubmoduleColor, setEditSubmoduleColor] = useState<string>("");
  const [editSubmoduleUserExtension, setEditSubmoduleUserExtension] =
    useState<any>(null);

  // Edit states for submodule children
  const [editingSubmoduleChild, setEditingSubmoduleChild] = useState<any>(null);
  const [editSubmoduleChildName, setEditSubmoduleChildName] =
    useState<string>("");
  const [editSubmoduleChildDescription, setEditSubmoduleChildDescription] =
    useState<string>("");
  const [editSubmoduleChildColor, setEditSubmoduleChildColor] =
    useState<string>("");
  const [editSubmoduleChildUserExtension, setEditSubmoduleChildUserExtension] =
    useState<any>(null);
  const [editSubmoduleChildSubmoduleId, setEditSubmoduleChildSubmoduleId] =
    useState<string>("");

  const fetchSubmodules = useCallback(async () => {
    if (!module?.id) return;
    try {
      const response = await ListSubmodules({
  ...
  const handleDeleteSubmoduleChild = useCallback(
    async (child: any) => {
      if (
        globalThis.confirm(
          `Are you sure you want to delete submodule child "${child.name}"?`,
        )
      ) {
  ...
  return (
    <Modal show={show} onHide={onHide} size="xl">
      ...
      <Modal.Body>
                  ...
                  value={
                    newSubmoduleUserExtension
                      ? {
                          ...
                          label:
                            extensions.find(
                              (ext) => ext.id === newSubmoduleUserExtension,
                            )?.display_name || "",
                        }
                      : null
                  }
                  onChange={(option) =>
                    setNewSubmoduleUserExtension(option?.value || null)
                  }
                ...
                <>
                  ...
                  <Form.Group className="mt-2">
                    ...
                    <Form.Control
                      ...
                      onChange={(e) =>
                        setNewSubmoduleChildDescription(e.target.value)
                      }
                      placeholder="Description"
                      rows={2}
                    />
                  ...
                  <Form.Group className="mt-2">
                    ...
                    <div className="d-flex gap-2">
                      <Form.Control
                        ...
                        onChange={(e) =>
                          setNewSubmoduleChildColor(e.target.value)
                        }
                        style={{ width: "50px" }}
                      />
                      <Form.Control
                        ...
                        newSubmoduleChildUserExtension
                          ? {
                              ...
                              label:
                                extensions.find(
                                  (ext) =>
                                    ext.id === newSubmoduleChildUserExtension,
                                )?.display_name || "",
                            }
                          : null
                      }
                      onChange={(option) =>
                        setNewSubmoduleChildUserExtension(option?.value || null)
                      }
                      options={extensions.map((ext) => ({
                        value: ext.id,
                        label: ext.display_name,
        ...
        {editingSubmodule && (
          <Modal
            show={!!editingSubmodule}
            onHide={() => setEditingSubmodule(null)}
            size="lg"
          >
            <Modal.Header closeButton>
                  ...
                  value={
                    editSubmoduleUserExtension
                      ? {
                          ...
                          label:
                            extensions.find(
                              (ext) => ext.id === editSubmoduleUserExtension,
                            )?.display_name || "",
                        }
                      : null
                  }
                  onChange={(option) =>
                    setEditSubmoduleUserExtension(option?.value || null)
                  }
                  options={extensions.map((ext) => ({
                    value: ext.id,
                    label: ext.display_name,
        ...
        {editingSubmoduleChild && (
          <Modal
            show={!!editingSubmoduleChild}
            onHide={() => setEditingSubmoduleChild(null)}
            size="lg"
          >
            <Modal.Header closeButton>
            ...
            <Modal.Body>
              ...
              <Form.Group className="mt-2">
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    setEditSubmoduleChildDescription(e.target.value)
                  }
                  placeholder="Description"
                  rows={2}
                />
                    ...
                    editSubmoduleChildUserExtension
                      ? {
                          ...
                          label:
                            extensions.find(
                              (ext) =>
                                ext.id === editSubmoduleChildUserExtension,
                            )?.display_name || "",
                        }
                      : null
                  }
                  onChange={(option) =>
                    setEditSubmoduleChildUserExtension(option?.value || null)
                  }
                  options={extensions.map((ext) => ({
                    value: ext.id,
                    label: ext.display_name,
            ...
            <Modal.Footer>
              <Button
                variant="secondary"
                onClick={() => setEditingSubmoduleChild(null)}
              >
                Cancel
              </Button>
