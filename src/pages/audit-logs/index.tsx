...
import {
  ...
  AuditFilterNode,
  AuditFilterService,
} from "@config/auditFilterConfig";
import { StatsCardData } from "@components/GenericStatsCards";
import AuditLogSidebar, {
  AuditSidebarField,
} from "@components/AuditLogSidebar";
  ...
  return {
    ...
    render: (row: Record<string, unknown>) => {
      ...
      return (
        <span
          title={summary
            .map((c: any) => `${c.field}: ${c.type} ${c.new || c.old || ""}`)
            .join("\n")}
        >
...
const AuditLogColumnsByModule: Record<
  string,
...
> = {
  ...
  Automation: [
    {
      ...
      type: "text",
      accessor: (row: Record<string, unknown>) =>
        (row.formatted_timestamp as string) ||
        (row.created_at
          ? moment(String(row.created_at)).format("MMM D, YYYY HH:mm:ss")
          : null) ||
        "—",
      emptyValue: "—",
    },
    {
      ...
      type: "text",
      accessor: (row: Record<string, unknown>) =>
        (row.formatted_action as string) ||
  ...
  CRM: [
    {
      ...
      type: "text",
      accessor: (row: Record<string, unknown>) =>
        (row.created_at_formatted as string) ||
        (row.created_at
          ? moment(String(row.created_at)).format("YYYY-MM-DD HH:mm:ss")
          : null) ||
        "—",
      emptyValue: "—",
    },
    {
      ...
      type: "text",
      accessor: (row: Record<string, unknown>) =>
        (row.action_display as string) || formatLabel(row.event) || "—",
      emptyValue: "—",
    },
    {
      key: "entity_type",
    ...
    },
    {
      ...
      accessor: (row: Record<string, unknown>) => {
        ...
        if (row.user_display != null && row.user_display !== "")
          return String(row.user_display);
        if (u && typeof u === "object" && !Array.isArray(u))
          return (u as any).display_name || (u as any).name || "—";
  ...
  "Main App": [
    ...
    },
    {
      ...
      type: "text",
      accessor: (row: Record<string, unknown>) =>
        formatLabel((row.record_type ?? row.resource_type) as string) || "—",
      emptyValue: "—",
    },
    {
      key: "user_id",
...
const DEFAULT_SIDEBAR_FIELDS =
  AUDIT_SIDEBAR_FIELDS_BY_MODULE["Staff management"];

function getSidebarFieldValue(
  ...
  if (key === "resource_type")
    return formatLabel(row.record_type ?? row.resource_type);
  if (key === "formatted_timestamp") {
    const v = row.formatted_timestamp;
    if (v != null && v !== "") return String(v);
    if (row.created_at)
      return moment(String(row.created_at)).format("MMM D, YYYY HH:mm:ss");
    return "—";
  }
  ...
  if (key === "created_at_formatted") {
    ...
    if (v != null && v !== "") return String(v);
    if (row.created_at)
      return moment(String(row.created_at)).format("YYYY-MM-DD HH:mm:ss");
    return "—";
  }
  ...
  if (key === "changes_summary") {
    ...
    if (!summary?.length) return "—";
    return `${summary.length} change${summary.length !== 1 ? "s" : ""}: ${summary
      .map((c) => c.field)
      .filter(Boolean)
      .join(", ")}`;
  }
...
function getTimestampParamKeys(
  timestamp: string | [string, string] | undefined,
): [string, string] {
...
function getSidebarFieldsForModule(
  ...
  const config =
    (moduleName && AUDIT_SIDEBAR_FIELDS_BY_MODULE[moduleName]) ||
    DEFAULT_SIDEBAR_FIELDS;
...
function normalizeCrmChanges(changes: unknown): ChangeItem[] {
  ...
  if (
    ...
    return Object.entries(obj).map(([key, val]) => ({
      ...
      type: "updated",
      old:
        val && typeof val === "object" && "old" in val ? format(val.old) : "—",
      new:
        val && typeof val === "object" && "new" in val ? format(val.new) : "—",
    }));
  }
  return [];
}
...
const AuditLogsNewPage = () => {
  ...
  const [selectedAuditModule, setSelectedAuditModule] =
    useState<AuditFilterNode | null>(null);
  const [selectedAuditService, setSelectedAuditService] =
    useState<AuditFilterService | null>(null);
  ...
  const [auditLogsSummary, setAuditLogsSummary] = useState<Record<
    string,
    number
  > | null>(null);
  const [auditUserOptions, setAuditUserOptions] = useState<
  ...
  const [selectedRow, setSelectedRow] = useState<Record<
    ...
    unknown
  > | null>(null);
  const [showAuditDateCustomModal, setShowAuditDateCustomModal] =
    useState(false);
  ...
  const sidebarFields: AuditSidebarField[] = selectedRow
    ? [
        {
          label: "Category",
          value: formatLabel(selectedRow.resource_type as string),
        },
        {
          label: "Subcategory",
          value: formatLabel(selectedRow.formatted_action as string),
        },
        ...
        {
          label: "Login Type",
          value: (selectedRow.login_type as string) || "—",
        },
        {
          label: "User Agent",
          value: (selectedRow.user_agent as string) || "—",
        },
        {
          label: "IP Address",
          value: (selectedRow.ip_address as string) || "—",
        },
      ]
    : [];
  ...
  const visibleAuditServices = useMemo(() => {
    ...
    const perms = session?.user?.permissions ?? [];
    return selectedAuditModule.services.filter((s) =>
      perms.includes(s.isShow ?? ""),
    );
  ...
  const handleAuditModuleChange = useCallback(
    (moduleName: string) => {
      ...
      const node =
        visibleAuditModules.find((n) => n.moduleName === moduleName) ?? null;
  ...
  const handleAuditServiceChange = useCallback(
    (serviceName: string) => {
      ...
      const svc =
        visibleAuditServices.find((s) => s.serviceName === serviceName) ?? null;
  ...
  useEffect(() => {
    ...
    GetHierarchyData(moduleSlug)
      .then((data: any) => {
        if (cancelled || !data?.extensions || !Array.isArray(data.extensions))
          return;
        setAuditUserOptions(
  ...
  useEffect(() => {
    if (
      !selectedAuditModule ||
      typeof selectedAuditModule.endpoint !== "function"
    )
      return;
    const moduleSlug =
      selectedAuditModule?.moduleSlug ??
    ...
    if (selectedAuditService) {
      if (selectedAuditService.serviceKey)
        params[selectedAuditService.serviceKey] =
          selectedAuditService.serviceValue;
      else params.service = selectedAuditService.serviceName;
    }
    if (selectedAuditUser) {
      const userKey =
        selectedAuditService?.userKey ??
        selectedAuditModule?.userKey ??
        "user_id";
      params[userKey] = selectedAuditUser;
    }
    if (selectedAuditAction) {
      if (selectedAuditService?.actionKey)
        params[selectedAuditService.actionKey] = selectedAuditAction;
      else params.action = selectedAuditAction;
    }

    const timestampConfig = selectedAuditModule?.timestamp;
    const [startKey, endKey] = getTimestampParamKeys(
      timestampConfig as [string, string],
    );
  ...
  const dataList = useMemo(() => {
    ...
    if (moduleName === "Main App") {
      return raw.map((row: any) => {
        ...
        const ext =
          row.user_extension != null ? String(row.user_extension) : "";
        const userDisplay =
          auditUserOptions.find((o) => String(o.value) === ext)?.label ??
          (row.user_extension && typeof row.user_extension === "object"
            ? (row.user_extension as any).display_name ||
              (row.user_extension as any).name
            : null);
        return {
          ...row,
          ...
          formatted_action:
            ...
            (row.action ? formatLabel(row.action) : null) ??
            row.action,
          changes_summary: row.changes_summary ?? derived,
          ...(userDisplay != null && userDisplay !== ""
    ...
    if (moduleName === "CRM") {
      return raw.map((row: any) => {
        ...
        return {
          ...row,
          changes_summary:
            row.changes_summary ?? normalizeCrmChanges(row.changes) ?? [],
          ...(userDisplay != null && userDisplay !== ""
            ? { user_display: userDisplay }
            : {}),
        };
  ...
  const changesSummary =
    (selectedRow?.changes_summary as ChangeItem[] | undefined) ?? [];
  ...
  const filterPills = useMemo(() => {
    ...
    pills.push({
      ...
      dropdownOptions: [
        ...
        ...(actionOptions?.map((a) => ({
          label: a,
          value: a,
          onClick: () => setSelectedAuditAction(a ?? ""),
        })) ?? []),
      ],
    });
    pills.push({
      ...
      label:
        auditStartDate && auditEndDate
          ? auditStartDate === auditEndDate
            ? moment(auditStartDate).format("MMM D, YYYY")
            : `${moment(auditStartDate).format("MMM D")} - ${moment(auditEndDate).format("MMM D, YYYY")}`
          : "Select Audit date",
      showDropdown: true,
      dropdownOptions: [
        {
        ...
        },
        {
          ...
          onClick: () => {
            ...
            setAuditEndDate(moment().format("YYYY-MM-DD"));
          },
        },
        {
          label: "Last 30 Days",
        ...
        },
        {
          ...
          value: "custom",
          onClick: () => {
            setCustomStartDate(auditStartDate || "");
            setCustomEndDate(auditEndDate || "");
            setShowAuditDateCustomModal(true);
          },
        },
      ],
    });
    ...
    if (showUserPill) {
      pills.push({
        id: "audit_user",
        label: selectedAuditUser
          ? auditUserOptions.find((o) => o.value === selectedAuditUser)
              ?.label || selectedAuditUser
          : "Select User",
        showDropdown: true,
        searchable: true,
        dropdownOptions: [
          ...
          ...auditUserOptions.map((o) => ({
            label: o.label,
            value: o.value,
            onClick: () => setSelectedAuditUser(o.value),
          })),
        ],
      });
    }
  ...
  return (
    <>
        ...
        <div style={{ padding: "20px" }}>
          ...
          >
            <GenericTable
              ...
              columns={getColumnsForModule(
                selectedAuditModule?.moduleName ?? null,
              )}
              actions={[]}
              showActions={false}
              selectable={false}
      ...
      <Modal
        show={showAuditDateCustomModal}
        onHide={() => setShowAuditDateCustomModal(false)}
        centered
      >
        ...
        <Modal.Footer>
          <Button
            ...
            onClick={() => setShowAuditDateCustomModal(false)}
          >
            Cancel
          </Button>
          <Button
