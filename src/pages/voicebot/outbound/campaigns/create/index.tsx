...
const CampaignCreatePage = (props: CampaignFormPageProps) => {
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      ...
      setCompanies(opts);
      if (opts.length && !form.company_id && !editCampaignId) setForm((f) => ({ ...f, company_id: opts[0].id }));
    } catch {
      setCompanies([]);
    } finally {
  ...
  const fetchVoicebots = useCallback(async (companyId: string) => {
    ...
    try {
      ...
      const list = Array.isArray(res) ? res : (res as { results?: { id?: number; name?: string }[] })?.results ?? (res as { data?: { id?: number; name?: string }[] })?.data ?? [];
      setVoicebots((Array.isArray(list) ? list : []).map((v) => ({ id: v.id ?? "", name: (v as { name?: string }).name ?? "" })));
    } catch {
      setVoicebots([]);
    }
  }, []);
  ...
  }, [isEditMode, editCampaignId, editCompanyId]);
  ...
  const validationItems = [
    { id: "basic", label: "Basic Information", checked: !!(form.name?.trim() && form.company_id && (form.voicebot_id != null || true)) },
    { id: "targets", label: "Target Numbers", checked: targetCount > 0 },
    { id: "script", label: "Script & Greeting", checked: !!(form.campaign_script ?? "").trim() },
  ];
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              {isLastTab ? (
                <Button
                  type="button"
                  variant="primary"
                  disabled={submitting || !form.company_id || !form.name?.trim() || (isEditMode ? false : (!(form.campaign_script ?? "").trim() || targetCount === 0))}
