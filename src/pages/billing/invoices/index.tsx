    ...
    <Form onSubmit={handleSubmit}>
      ...
      <div className="d-flex gap-2 mt-3">
        <Button
          ...
          variant="success"
          disabled={
            !cardComplete || isProcessing || isCreateInvoicePaymentPending
          }
          className="w-100"
        >
...
const InvoiceList = () => {
  ...
  const [selectedDescription] = useState<string>("");
      ...
      },
      {
        ...
        render: (row) => (
          <span>
            ...
            {formatNumber(Number.parseFloat(row?.total_amount || "0"))}
          </span>
        ),
      },
      {
        ...
        render: (row) => (
          <span>
            {row.currency_code || "AED"}{" "}
            {formatNumber(Number.parseFloat(row?.amount_due || "0"))}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        show: (row: InvoiceData) =>
          (row.status === STATUS_PENDING ||
            ...
            row.status === STATUS_PARTIALLY_PAID) &&
    ...
    () => [
      ...
      },
      {
        ...
        onChange: (value) =>
          setPendingFilters((prev) => ({
            ...prev,
            invoice_date_from: value || undefined,
          })),
      },
      {
        id: "invoice_date_to",
  ...
  const getEffectiveProductPrice = useCallback(
    (
      productId: string,
      companyId?: string,
    ): { price: string; currency: string; includesVat: boolean } =>
  ...
  useEffect(() => {
    if (showPaymentModal && paymentMethods.length > 0) {
      ...
      const defaultCard = paymentMethods.find(
        (method) => method.is_default && method.type === "card",
      );
      if (defaultCard) {
        setSelectedCardId(defaultCard.id);
      } else {
        const firstCard = paymentMethods.find(
  ...
  const [selectedInvoiceSidebar, setSelectedInvoiceSidebar] =
    useState<InvoiceData | null>(null);
  ...
  const _fetchInvoices = useCallback(
    async (page = 1, perPage = 15, search = "") => {
      try {
        ...
        return {
          ...
          page:
            response?.pagination?.current_page || response?.pagination?.page,
          per_page:
            response?.pagination?.per_page || response?.pagination?.limit,
  ...
  const _handleFiltersChange = useCallback(
    (filters: Record<string, unknown>) => {
      setCurrentFilters(filters as typeof currentFilters);
    },
    [],
  );
  ...
  useEffect(() => {
    if (exchangeRates.length > 0 && !isLoadingExchangeRates) {
      ...
      if (
        ...
        newInvoice.currency_code !== "USD" &&
        newInvoice.items.length > 0
      ) {
        const selectedCompany = companies.find(
          (c) => c.id.toString() === newInvoice.company_id,
        );
        const companyVatRate = selectedCompany?.profile?.vat_rate
          ? Number.parseFloat(selectedCompany.profile.vat_rate)
          : 0;
      ...
      if (
        ...
        selectedInvoice.currency_code !== "USD" &&
        selectedInvoice.items.length > 0
      ) {
        const selectedCompany = companies.find(
          (c) => c.id.toString() === selectedInvoice.company_id,
        );
        const companyVatRate = selectedCompany?.profile?.vat_rate
          ? Number.parseFloat(selectedCompany.profile.vat_rate)
          : 0;
  ...
  const _transformInvoiceForAPI = (
    invoice: InvoiceCreateUpdatePayload,
  ): InvoiceCreateUpdateAPIPayload => {
    return {
      ...invoice,
    ...
    createInvoicePayment(
      {
        ...
        currency: (
          selectedInvoiceForPayment.currency_code || "USD"
        ).toLowerCase(),
        payment_method_id: selectedCardId,
        invoice_id: selectedInvoiceForPayment.id,
        customer_id: Number.parseInt(
          selectedInvoiceForPayment.company_id || "0",
        ),
      },
      {
        onSuccess: (paymentResult: any) => {
  ...
  const _generateInvoiceHTML = useCallback(
    (invoice: InvoiceData) => {
      // Get company VAT rate
      const companyVatRate = invoice.company?.profile?.vat_rate
        ? Number.parseFloat(invoice.company.profile.vat_rate)
        : 0;
              ...
              <tbody>
                ${invoice.items
                  .map((item) => {
                    ...
                    const productInfo = getEffectiveProductPrice(
                    ...
                    <tr>
                      ...
                      <td>${item.quantity}</td>
                      <td>${(Number.parseFloat(item.unit_price) * exchangeRate).toFixed(2)}</td>
              ...
              <div class="totals">
                <div class="totals-content">
                  <div class="totals-row">
                    ...
                    <span class="totals-amount">${invoice.currency_code || "AED"} ${Number.parseFloat(invoice.subtotal || "0").toFixed(2)}</span>
                  </div>
                  <div class="totals-row">
                    <span class="totals-label">VAT (${isVatExempt ? "Exempt" : companyVatRate + "%"}):</span>
                    <span class="totals-amount">${invoice.currency_code || "AED"} ${Number.parseFloat(invoice.tax_amount || "0").toFixed(2)}</span>
                  </div>
                  <div class="totals-row total-final">
                    <span class="totals-label">Total Amount:</span>
                    <span class="totals-amount">${invoice.currency_code || "USD"} ${Number.parseFloat(invoice.total_amount || "0").toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="notes-section">
                ...
                <div class="notes-content">${invoice.terms_conditions
                  ...
                  .map((line) => {
                    ...
                    const execResult = numberedListRe.exec(trimmedLine);
                    if (execResult) {
                      return `<div class="d-flex align-items-start mb-1"><span class="me-2 text-primary fw-bold">${execResult[1]}</span><span>${execResult[2]}</span></div>`;
                    }
  ...
  return (
    <React.Fragment>
        ...
        <Card className="filter-bar-card">
          <Card.Body className="filter-bar-body">
            <div className="filter-bar-tabs">
              ...
              <Button
                variant={
                  activeStatusTab === "paid" ? "primary" : "outline-secondary"
                }
                className="filter-bar-tab"
                onClick={() => {
      ...
      <GenericSidebar
        ...
        title={
          selectedInvoiceSidebar
            ? `Invoice #${selectedInvoiceSidebar.invoice_number}`
            : "Invoice Details"
        }
        subtitle={selectedInvoiceSidebar?.company?.name ?? ""}
        metadata={
          selectedInvoiceSidebar?.due_date
            ? `Due: ${moment(selectedInvoiceSidebar.due_date).format("DD-MMM-YYYY")}`
            : undefined
        }
        width="400px"
        sections={[
            ...
            fields: [
              {
                ...
                value: selectedInvoiceSidebar
                  ? `#${selectedInvoiceSidebar.invoice_number}`
                  : "N/A",
              },
              {
                label: "Status",
          ...
          },
          {
            ...
            fields: [
              {
                label: "Company",
                value: selectedInvoiceSidebar?.company?.name ?? "N/A",
              },
              {
                label: "Country",
                value: selectedInvoiceSidebar?.company?.country ?? "N/A",
              },
        ...
        actions={[
          ...
          },
          {
            ...
            show: !!(
              selectedInvoiceSidebar &&
              (selectedInvoiceSidebar.status === STATUS_PENDING ||
                selectedInvoiceSidebar.status === STATUS_OVERDUE ||
                selectedInvoiceSidebar.status === STATUS_PARTIALLY_PAID) &&
              session?.user?.permissions?.includes("pay-invoices-billing")
            ),
            onClick: () => {
              if (selectedInvoiceSidebar) {
      ...
      {showPaymentModal && selectedInvoiceForPayment && (
        <Modal show={showPaymentModal} onHide={closePaymentModal} size="lg">
                ...
                <div className="row">
                  <div className="col-md-6">
                    ...
                    <p>
                      ...
                      {moment(selectedInvoiceForPayment.invoice_date).format(
                        "DD-MMM-YYYY",
                      )}
                    </p>
                    <p>
                      <strong>Due Date:</strong>{" "}
                      {selectedInvoiceForPayment.due_date
                        ? moment(selectedInvoiceForPayment.due_date).format(
                            "DD-MMM-YYYY",
                          )
                        : "N/A"}
                    </p>
                  ...
                  <div className="col-md-6">
                    <p>
                      ...
                      {formatNumber(
                        Number.parseFloat(
                          selectedInvoiceForPayment.subtotal || "0",
                        ),
                      )}
                    </p>
                    <p>
                      ...
                      {formatNumber(
                        Number.parseFloat(
                          selectedInvoiceForPayment.tax_amount || "0",
                        ),
                      )}
                    </p>
                    <p>
              ...
              <div className="nav nav-tabs mb-3" role="tablist">
                <div className="nav-item">
                  <button
                    ...
                    type="button"
                    role="tab"
                    aria-selected={activePaymentTab === "saved-cards"}
                  >
                    Saved Cards
                  </button>
                </div>
                <div className="nav-item">
                  <button
                    ...
                    type="button"
                    role="tab"
                    aria-selected={activePaymentTab === "direct-payment"}
                  >
                    Direct Payment
                  </button>
                </div>
                  ...
                  <div className="tab-pane active">
                    {paymentMethods.length > 0 ? (
                          ...
                          {paymentMethods
                            ...
                            .map((method) => (
                              <label
                                ...
                                aria-label={`Select card ${method.card?.brand ?? "Card"} ending ${method.card?.last4 ?? ""}`}
                              >
                                <div className="card-body">
                      ...
                      <div className="text-center py-4">
                        ...
                        <small className="text-muted">
                          Use the Direct Payment tab to add a new card
                        </small>
                      </div>
                    )}
                  </div>
              ...
              <>
                <Row>
                  <Col md={6}>
                    ...
                    <p className="mb-3">
                      <b>Email:</b>{" "}
                      {selectedInvoiceForView?.company?.reseller?.email || ""}
                    </p>
                  </Col>
                  <Col md={6}>
                    <div>
                    ...
                    {Number(selectedInvoiceForView.amount_due) > 0 && (
                      ...
                      >
                        ...
                        <span className="fw-bold text-danger">
                          ...
                          {formatNumber(
                            Number.parseFloat(
                              String(selectedInvoiceForView.amount_due ?? 0),
                            ),
                          )}
                        </span>
                      </div>
                ...
                <Row>
                  <Col md={6}>
                    ...
                    <div className="border p-3 rounded bg-light mb-3">
                      ...
                      <p className="mb-3">
                        {selectedInvoiceForView?.company?.country || ""}
                      </p>
                    ...
                    >
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td className="p-2">
                            {selectedInvoiceForView.invoice_number || "N/A"}
                          </td>
                        </tr>
                        <tr>
                          <td
                        ...
                        <tr>
                          ...
                          <td className="p-2">
                            {selectedInvoiceForView.invoice_date
                              ...
                              : "N/A"}{" "}
                            -{" "}
                            {(selectedInvoiceForView?.end_date ??
                        ...
                        <tr>
                          ...
                          <td className="p-2">
                            {selectedInvoiceForView?.due_date
                              ? moment(selectedInvoiceForView?.due_date).format(
                                  GlobalDateFormat,
                                )
                              : ""}
                          </td>
