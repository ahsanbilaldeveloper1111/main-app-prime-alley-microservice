...
const AccountOverview = () => {
  ...
  useEffect(() => {
    if (companyDetails && showBillingEditModal) {
      setBillingInfo({
        ...
        country:
          companyDetails.country || companyDetails.profile?.country || "",
        profile: {
  ...
  const getDashboardCounters = async (
    params: { crm_company_id?: string | number } = {},
  ) => {
  ...
  return (
    <React.Fragment>
          ...
          <Form.Select
            ...
            onChange={(e) => {
              ...
              setSelectedCompanyName(
                getCompanyByCrmId(e.target.value, companies) ?? "",
              );
            }}
          >
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      {formatNumber(dashboardCounters?.invoices?.total_amount)}
                    </h4>
              ...
              <Card.Body className="p-3">
                ...
                <div className="d-flex justify-content-between align-items-center py-2">
                  ...
                  <span className="fw-semibold" style={{ fontSize: "0.9rem" }}>
                    {companyDetails?.profile?.currency}{" "}
                    {formatNumber(
                      companyDetails?.profile?.outstanding_invoices,
                    )}
                  </span>
                </div>
                  ...
                  recentInvoices.map((invoice: any, index: number) => {
                    const getStatusIcon = (iconColor: string) => {
                      ...
                      if (status === "paid") {
                        ...
                        return (
                          <FileText size={10} style={{ color: iconColor }} />
                        );
                      }
                    };
                    ...
                    return (
                      <div
                        key={invoice.id}
                        className={`py-1 ${index < recentInvoices.length - 1 ? "border-bottom" : ""}`}
                      >
                        <div className="d-flex justify-content-between align-items-center">
                          ...
                          >
                            ...
                            {formatNumber(invoice.total_amount || 0)}
                          </small>
                        </div>
                      </div>
                    );
                ...
                {paymentHistory.length > 0 ? (
                  paymentHistory.map((payment: any, index: number) => {
                    ...
                    return (
                      <div
                        key={payment.id}
                        className={`d-flex gap-2 py-1 ${index < paymentHistory.length - 1 ? "border-bottom" : ""}`}
                      >
                        {getIcon()}
      ...
      <Modal
        show={showBillingEditModal}
        onHide={() => setShowBillingEditModal(false)}
        size="lg"
        centered
      >
                  ...
                  <ThemeSelect
                    value={(() => {
                      const countryValue =
                        ...
                        companyDetails?.profile?.country ||
                        "";
      ...
      <Modal
        ...
        onHide={() => setShowManageAccountModal(false)}
        size="lg"
        centered
      >
        <Modal.Header closeButton>
