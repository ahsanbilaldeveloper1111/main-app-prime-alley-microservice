...
import {
  ...
  DeleteUser,
  UpdatePassword,
} from "@utils/tms/tmsUserManagement";
...
const TmsUserManagement = () => {
  ...
  const handleUserTypeChange = useCallback(
    async (userId: number, newUserType: string, currentUserType: string) => {
      ...
      if (!validUserTypes.includes(currentUserType)) {
        toast.warning(
          `Cannot update user type. Current type "${currentUserType}" is not supported.`,
        );
        return;
      }
  ...
  const handleUserRankChange = useCallback(
    async (userId: number, newUserRank: string, currentUserRank: string) => {
      ...
      if (!validRanks.includes(newUserRank)) {
        toast.warning(
          `Cannot update user rank. Selected rank "${newUserRank}" is not supported.`,
        );
        return;
      }
      ...
      try {
        ...
        if (response) {
          const selectedRank = ListRanks.find(
            (rank) => rank.value === newUserRank,
          );
          toast.success(
            `User rank updated to ${selectedRank?.label || newUserRank} successfully!`,
          );
          setRefreshKey((prev) => prev + 1); // Refresh the table
        }
      } catch (error) {
  ...
  const columns: Column[] = useMemo(
    () => [
      ...
      // },
      {
        ...
        cell: (row: any) => (
          <div className="text-uppercase">{row.user_type}</div>
        ),
        sortable: false,
      },
      {
        ...
        <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
          ...
          <Modal.Body>
            <p>
              ...
              <b className="text-danger">{confirmDelete}</b> user?
            </p>
            <p>
            ...
            <input
              ...
              id="confirmDelete"
              value={confirmDeleteUser}
              onChange={(e) => setConfirmDeleteUser(e.target.value)}
        ...
        >
          ...
          <Modal.Body>
            <p>
              ...
              <b>{selectedUserForPassword?.username}</b>
            </p>
