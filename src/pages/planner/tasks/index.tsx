...
function getInitialVisibleTabIds(): string[] {
  ...
  try {
    ...
    const filtered = (parsed as string[]).filter((id) =>
      validIds.includes(id as any),
    );
...
const TasksListingPage = () => {
  ...
  const mapApiTaskToTask = useCallback(
    (apiTask: ApiTask): Task => {
      ...
      const isOverdue =
        dueMoment && dueMoment.isBefore(moment(), "day") && !isCompleted;
      ...
      router.push(
        {
          ...
          query: { ...router.query, tab: visibleTabIds[0] },
        },
        undefined,
        { shallow: true },
      );
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      if (filters.status && filters.status !== "All Status") {
        const statusId = tasksRef.current.find(
          (t) => t.rawData?.status?.name === filters.status,
        )?.rawData?.status?.id;
    ...
    () => [
      {
        ...
        render: (row) => (
          <button
            ...
            title={
              row.status === "completed" ? "Mark incomplete" : "Mark complete"
            }
            style={{
      ...
      },
      {
        ...
        render: (row) => {
          if (!row.due_date)
            return <span style={{ ...CELL_STYLE, color: "#9ca3af" }}>—</span>;
          const overdue =
            moment(row.due_date).isBefore(moment()) &&
            row.status !== "completed";
          const isToday = moment(row.due_date).isSame(moment(), "day");
          const isTomorrow = moment(row.due_date).isSame(
            moment().add(1, "day"),
            "day",
          );
          const label = isToday
      ...
      },
      {
        ...
        type: "custom",
        render: (row) => (
          <span style={CELL_STYLE}>{row.repeat_status || "—"}</span>
        ),
      },
    ],
  ...
  const filterFields: FilterField[] = [
    {
      ...
      value: fForm.project,
      onChange: (v) =>
        setFForm((p) => ({ ...p, project: v ?? "All Projects" })),
    ...
    },
    {
      ...
      label: "Assignee",
      type: "multi-select",
      value: fForm.assignee.map((ext) => ({
        value: ext,
        label: assigneeOptions.find((o) => o.value === ext)?.label ?? ext,
      })),
      onChange: (opts: Array<{ value: string; label: string }>) =>
        setFForm((p) => ({
          ...p,
          assignee: opts?.length ? opts.map((o) => o.value) : [],
        })),
  ...
  const filterPills: FilterPill[] = [
    ...
    },
    {
      ...
      label: fForm.task_type
        ? TASK_TYPE_OPTIONS.find((o) => o.value === fForm.task_type?.value)
            ?.label || "Task type"
        : "Task type",
      icon: <ChevronDown size={12} />,
      showDropdown: false,
    ...
    <React.Fragment>
      ...
      >
            ...
            return (
              <>
                ...
                {allTabs.map((tab) => (
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor:
                        activeTab === tab.id ? "#f7f2f7" : "#fff",
                      borderBottom:
                        activeTab === tab.id ? "none" : "1px solid #8A8A8A",
                      color: "#141414",
                      fontSize: 13,
                ...
                {!hasAllViews && (
                  <button
                    onClick={() =>
                      setVisibleTabIds(POSSIBLE_TABS.map((t) => t.id))
                    }
                    style={{
                      display: "inline-flex",
                      alignItems: "center",
        ...
        <Modal
          ...
          onHide={() => setShowAddViewModal(false)}
          centered
        >
          ...
          <Modal.Body>
            <p className="text-muted small mb-3">
              Toggle which views appear in the tab bar. At least one must be
              visible.
            </p>
        ...
        <div style={{ flex: 1, overflow: "hidden" }}>
          <GenericTable
            ...
            onSort={(col, dir) =>
              setPager((p) => ({ ...p, sortCol: col, sortDir: dir }))
            }
            onFirstColumnClick={(row) =>
              router.push(`/planner/tasks/task-detail?id=${row.id}`)
            }
            loading={loading}
      ...
      <GenericFilterSidebar
        ...
        onApply={() => {
          ...
          if (fForm.project && fForm.project !== "All Projects")
            f.project = fForm.project;
          if (fForm.assignee?.length) f.assignee = fForm.assignee;
          if (fForm.status && fForm.status !== "All Status")
            f.status = fForm.status;
