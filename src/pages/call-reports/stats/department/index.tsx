...
const CallStatsDepartment = () => {
    ...
    useEffect(() => {
        console.log('CallStatsDepartment component mounted');
        ...
        return () => {
            console.log('CallStatsDepartment component unmounting');
        };
    }, []);
    ...
    const fetchCallLogs = useCallback(async (page = 1, perPage = 15, search = "") => {
        ...
        try {
            console.log('About to call ListCallLogs with params:', { page, perPage, search, filters: currentFilters, reportType: 'statsDepartment' });
            const response = await ListCallLogs({ page, perPage, search, filters: currentFilters, reportType: 'statsDepartment' }, 'call-logs/statsByDepartment');
      ...
      if (areFiltersReady) {
        ...
        const fetchCharts = async () => {
          ...
          try {
            ...
            console.log('About to call ListCallLogs for chart with params:', { page: 1, perPage: 15, search: "", filters: currentFilters, reportType: 'chartDepartment' });
        ...
        console.log('Conditions not met for chart fetch:', { 
          filtersReady, 
          hasCurrentFilters: !!currentFilters, 
          filtersKeys: currentFilters ? Object.keys(currentFilters) : [], 
          status, 
          hasSession: !!session 
        });
        setChartCalls(null);
    ...
    const handleOpenChartModal = (chartData: { series: any[]; categories: string[] } | null, title: string, dataType: 'calls' | 'time' | 'cost' | 'custom') => {
        console.log('Opening chart modal:', { title, dataType, hasChartData: !!chartData });
        if (chartData) {
