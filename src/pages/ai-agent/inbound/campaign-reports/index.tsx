...
/** Inbound call item from getAllInboundCalls (sip_calls / web_calls) */
interface InboundCallItem {
  session_id: string;
  sip_call_id?: string;
  sip_phoneNumber?: string;
  sip_trunkPhoneNumber?: string;
...
function inboundToSession(call: InboundCallItem): Session {
  const durationSec = call.call_duration_seconds ?? 0;
  const contact = (call.sip_phoneNumber ?? call.sip_trunkPhoneNumber ?? "").toString();
  const statusMap: Record<string, Session["status"]> = {
    completed: "Connected",
    ringing: "Queued",
    in_progress: "Connected",
  };
  const status = statusMap[call.call_status ?? ""] ?? "Queued";
  return {
    id: call.session_id,
    contact,
    status,
...
function inboundToSessionDetail(call: InboundCallItem): SessionDetail {
...
const AIMLCampaignReports = () => {
  ...
  const [inboundCalls, setInboundCalls] = useState<InboundCallItem[]>([]);
  ...
  const loadPage = useCallback(async (page: number) => {
    ...
    setError(null);
    try {
      const res = await getAllInboundCalls({}, { page, page_size: PAGE_SIZE });
      const data = res?.data as {
        count?: number;
        results?: { sip_calls?: InboundCallItem[]; web_calls?: InboundCallItem[] };
      };
      const sip = data?.results?.sip_calls ?? [];
      const web = data?.results?.web_calls ?? [];
      const list: InboundCallItem[] = [...sip, ...web];
      setInboundCalls(list);
      setSessions(list.map(inboundToSession));
                    ...
                    {loading ? (
                      ...
                      sessions.map((session, index) => (
                        <tr
                          ...
                          onClick={() => {
                            const call = inboundCalls[index];
                            if (call) setSelectedSession(inboundToSessionDetail(call));
