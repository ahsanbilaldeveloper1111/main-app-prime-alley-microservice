...
export type CrmInsightsOrdersSectionProps = {
...
export function CrmInsightsOrdersSection({
  ...
  const revenueChartData = useMemo(() => {
    if (!orderRevenue?.by_month?.length) return [];
    return aggregateOrderRevenueByMonthChartPoints(orderRevenue.by_month);
                ...
                <div className="d-flex align-items-center justify-content-end mt-2">
                  <span className="text-muted small" style={{ fontSize: "0.8rem" }}>
                    {orderSummary?.total_orders
                      ? `${((pendingCount / orderSummary.total_orders) * 100).toFixed(1)}% of total`
                      : "0% of total"}
                  </span>
                </div>
              </Card.Body>
