...
import { GlobalDateTimeFormat, formatDateTimeToLocal, ModuleSlug, getAutoTimezone, formatMinutesAndSeconds } from '@utils/Helper';
...
const donutDataLabelsFormatter = (value: number): string => `${value.toFixed(0)}%`;
...
function createCalledNumbersDropdownContent(
  ...
  return function CalledNumbersRender({ closeMenu }: { closeMenu: () => void }) {
    return (
      <CalledNumbersPillMenu value={value} onChange={onChange} onApply={onApply} closeMenu={closeMenu} />
    );
  };
}
...
const CallStatsCountry = () => {
  ...
  const [currentChartDataType, setCurrentChartDataType] = useState<ChartDataType>('custom');
  ...
  const [pendingStartDate, setPendingStartDate] = useState<string>((currentFilters as any)?.start_datetime || defaultFilters.pending.start_datetime);
  const [pendingEndDate, setPendingEndDate] = useState<string>((currentFilters as any)?.end_datetime || defaultFilters.pending.end_datetime);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const applyFilters = useCallback((updated: Record<string, any>) => {
    handleFiltersChange(updated);
    fetchCallLogs(1, rowsPerPage, '');
  }, [handleFiltersChange, rowsPerPage, fetchCallLogs]);

  const toggleStringIdInFilter = useCallback((key: 'extension_number' | 'department', id: string) => {
  ...
  const statsCardsData: StatsCardData[] = useMemo(() => {
    ...
    return [
      { title: 'Total Calls', value: summary.total_calls, subtitle: 'Total number of calls' },
      { title: 'Avg Ring Time', value: formatMinutesAndSeconds(summary.avg_ring_time), subtitle: 'Average ring time per call' },
      { title: 'Avg Duration', value: formatMinutesAndSeconds(summary.avg_duration), subtitle: 'Average call duration' },
      { title: 'Total Duration', value: formatMinutesAndSeconds(summary.total_duration), subtitle: 'Total duration of calls' },
    ];
  }, [dataLoaded, summary]);
  ...
  const dateRangeActiveLabel = useMemo(() => {
    ...
    if (s && e) return `${formatDateTimeToLocal(s, GlobalDateTimeFormat)} – ${formatDateTimeToLocal(e, GlobalDateTimeFormat)}`;
  ...
  const filterPills: FilterPill[] = useMemo(() => [
    {
      ...
      onClear: () => {
        ...
        applyFilters({ ...currentFilters, start_datetime: freshDefaults.current.start_datetime, end_datetime: freshDefaults.current.end_datetime });
      },
      dropdownContent: createDateRangeDropdownContent(
        ...
        () => { setIsDateRangeSet(true); applyFilters({ ...currentFilters, start_datetime: pendingStartDate, end_datetime: pendingEndDate }); },
      ),
    },
    {
      id: 'direction',
      ...
      dropdownOptions: [
        { label: 'All', value: '', onClick: () => { const u = { ...currentFilters }; delete u.is_incoming_only; applyFilters(u); } },
        { label: 'Incoming', value: 'true', onClick: () => applyFilters({ ...currentFilters, is_incoming_only: 'true' }) },
        { label: 'Outgoing', value: 'false', onClick: () => applyFilters({ ...currentFilters, is_incoming_only: 'false' }) },
      ],
    },
    {
      id: 'call_status',
      ...
      dropdownOptions: [
        { label: 'All', value: '', onClick: () => applyFilters({ ...currentFilters, call_status: '' }) },
    ...
    },
    {
      ...
      dropdownOptions: [
        { label: 'All', value: '', onClick: () => applyFilters({ ...currentFilters, extension_number: [] }) },
        ...((hierarchyDataExtensions as any[]) || []).map((ext: any) => ({
          label: ext.name,
          value: String(ext.id),
          onClick: () => toggleStringIdInFilter('extension_number', String(ext.id)),
        })),
      ],
    },
    {
      ...
      dropdownOptions: [
        { label: 'All', value: '', onClick: () => applyFilters({ ...currentFilters, department: [] }) },
        ...((hierarchyDataDepartments as any[]) || []).map((dept: any) => ({
          label: dept.name,
          value: String(dept.id),
          onClick: () => toggleStringIdInFilter('department', String(dept.id)),
        })),
      ],
    },
    {
      ...
      dropdownContent: createCalledNumbersDropdownContent(
        ...
        () => {
          const values = pendingCalledNumbers.split(',').map((v) => v.trim()).filter(Boolean);
          applyFilters({ ...currentFilters, called_numbers: values });
        },
      ),
    },
  ], [currentFilters, isDateRangeSet, dateRangeActiveLabel, directionLabel, statusLabel, trafficLabel, destinationLabel,
      extensionActiveLabel, departmentActiveLabel, calledNumbersActiveLabel,
      pendingStartDate, pendingEndDate, pendingCalledNumbers,
      hierarchyDataExtensions, hierarchyDataDepartments, applyFilters, toggleStringIdInFilter]);
  ...
  const tableToolbar: ToolbarConfig = useMemo(() => ({
    ...
    rightActions: showDateRange ? (
      <p className="mb-0 small">
        ...
        <span className="status-badge primary ms-2">{formatDateTimeToLocal(startDateTime, GlobalDateTimeFormat)}</span>
  ...
  function renderBarChart(
    ...
    return (
      <ChartBar
        ...
        onFullScreenClick={() => handleOpenChartModal(modalData, modalTitle, modalDataType)}
      />
    );
  }
    ...
    <React.Fragment>
      ...
      <div className="mt-4">
        ...
        >
          ...
          >
            <h6 className="mb-0 fw-semibold" style={{ fontSize: '14px', color: '#222' }}>
              Call Analytics
            </h6>
          </div>
          ...
          <div className="p-3">
            <Row className="g-3">
              ...
              <Col md={4}>
                ...
                >
                  <p className="text-muted mb-3 fw-semibold" style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      ...
                      <AnimatePresence mode="wait">
                        {activeTab === 'calls_chart' && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: -20 }}
                          >
                            {renderBarChart(chartCalls, 'calls', chartCalls, 'Calls by Country', 'calls')}
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </Tab>
                      ...
                      <AnimatePresence mode="wait">
                        {activeTab === 'duration_chart' && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: -20 }}
                          >
                            {renderBarChart(chartDuration, 'time', chartDuration, 'Duration by Country', 'time')}
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </Tab>
                      ...
                      <AnimatePresence mode="wait">
                        {activeTab === 'ring_chart' && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: -20 }}
                          >
                            {renderBarChart(chartRingTime, 'time', chartRingTime, 'Ring Time by Country', 'time')}
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </Tab>
