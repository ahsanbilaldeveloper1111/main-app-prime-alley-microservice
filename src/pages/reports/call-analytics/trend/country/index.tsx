...
const CallTrendCountry = () => {
    ...
    const fetchCallLogs = useCallback(async (page = 1, perPage = 15, search = "") => {
        ...
        try {
            const response = await ListCallLogs({ page, perPage, search, filters: currentFilters, reportType: 'trendStatsCountry', 
                moduleSlug: ModuleSlug.CALL_REPORTS }, 'call-logs/statsTrendByCountry').finally(() => {
      ...
      try {
        if (exportType === 'excel') {
          ...
          await DownloadStreamingExport(
            { filters: exportFilters, isExport: true, exportType ,reportType:'trendStatsCountry' }, 'call-logs/statsTrendByCountry',
            'trendStatsCountry'
          );
        }
      } catch (error: unknown) {
    ...
    return (
        <React.Fragment>
            <BreadcrumbItem mainTitle="" mainLink="" subTitle="Call Trend By Country" showPageLoader={showPageLoader} />
