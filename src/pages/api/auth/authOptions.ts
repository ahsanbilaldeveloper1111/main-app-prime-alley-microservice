...
import { NEXTAUTH_SESSION_MAX_AGE_SEC } from '../../../utils/nextAuthSessionConstants';
import { jwtPayloadStore, isLikelySessionIdHex } from '../../../utils/sessionStore';
...
declare module 'next-auth/jwt' {
  interface JWT {
    /** Keys jwtPayloadStore; mirrors signed cookie sessionId */
    sessionId?: string;
  }
}
...
export const authOptions: NextAuthOptions = {
  ...
  jwt: {
    async encode({ token = {}, secret, maxAge = NEXTAUTH_SESSION_MAX_AGE_SEC }) {
      const tok = token as JWT;
      const prev =
        typeof tok.sessionId === 'string' && isLikelySessionIdHex(tok.sessionId)
          ? tok.sessionId
          : undefined;
      const sessionId = prev ?? generateSessionId();

      const now = Math.floor(Date.now() / 1000);
      const maxAgeSec =
        typeof maxAge === 'number' ? maxAge : NEXTAUTH_SESSION_MAX_AGE_SEC;
      const exp = typeof tok.exp === 'number' ? tok.exp : now + maxAgeSec;

      const payloadForStore: Record<string, unknown> = {
        ...tok,
        sessionId,
        exp,
        iat: typeof tok.iat === 'number' ? tok.iat : now,
      };

      jwtPayloadStore.set(sessionId, payloadForStore);

      const secretStr =
        typeof secret === 'string' ? secret : (secret as Buffer).toString('binary');
      ...
      return signSmallPayload(
        {
          ...
          exp,
          iat: typeof tok.iat === 'number' ? tok.iat : now,
          id: typeof tok.id === 'string' ? tok.id : undefined,
        },
        secretStr
      );
    },
