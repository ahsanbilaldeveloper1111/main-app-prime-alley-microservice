...
const CtiDashboard = () => {
  ...
  const [callStatus, setCallStatus] = useState<'dialing' | 'connected' | 'onHold' | 'ended'>('dialing')
  ...
  const handleDial = async () => {
    if (dialedNumber.trim()) {
      setIsDialing(true)
      try {
        const response = await fetch(`${baseUrl}/api/cti/dial`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({
            cucmUsername: "admin", // You can make this configurable
            userAddress: "101", // You can make this configurable or get from context
            callingAddress: "101", // You can make this configurable or get from context
            calledAddress: dialedNumber,
            callingDeviceType: "SOFT_HARD", // You can make this configurable
            callingDeviceName: "WebCTI" // You can make this configurable
          })
        })

        if (response.ok) {
          const result = await response.json()
          console.log('Dial API response:', result)
          
          // Close dialer and show call status
          setShowDialer(false)
          setShowCallStatus(true)
          setCallStatus('dialing')
          
          // Simulate call progress (you can remove this when real-time updates are available)
          setTimeout(() => setCallStatus('connected'), 2000)
          
          setNotification({ type: 'success', message: `Call initiated to ${dialedNumber}` })
        } else {
          const errorData = await response.json()
          console.error('Dial API error:', errorData)
          setNotification({ type: 'danger', message: `Failed to initiate call: ${errorData.message || 'Unknown error'}` })
        }
      } catch (error) {
        console.error('Error calling dial API:', error)
        setNotification({ type: 'danger', message: 'Failed to initiate call: Network error' })
      } finally {
        setIsDialing(false)
      }
    }
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <Modal.Body>
            ...
            <Row>
                  <Col xs={12} md={4}>
                        ...
                        <div className="mb-4">
                              <h6 className="fw-bold mb-3">Available Extensions</h6>
                              <ul className="extensions-list">
                                {Object.keys(dnsMap).map((dn) => {
                                  const deviceList = Object.values(dnsMap[dn]?.devices || {})
                                  const cls = getCardLevelStatus(deviceList)
                                  const isOnline = cls === 'registered'
                                  
                                  return (
                                    <li key={dn} className="extension-item">
                                      <button
                                        type="button"
                                        className={`extension-button ${isOnline ? 'online' : 'offline'}`}
                                        onClick={() => setDialedNumber(dn)}
                                        disabled={!isOnline}
                                        title={`${dn} - ${isOnline ? 'Online' : 'Offline'}`}
                                      >
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowDialer(false)}>
            Close
          </Button>
        </Modal.Footer>
      </Modal>

      {/* Call Status Popup Modal */}
      <Modal
        show={showCallStatus}
      ...
      >
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            {/* New Call Button - Always visible */}
            <div className="mt-3 pt-3 border-top">
              <Button
                ...
                onClick={() => setShowDialer(true)}
              >
                {/* <i className="material-icons-two-tone me-2">add_call</i> */}
                New Call
              </Button>
            </div>
          </div>
